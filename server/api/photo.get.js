...
export default defineEventHandler(async (event) => {
  ...
  let rows;
  try {
    [rows] = await conn.execute("SELECT id, url, price_cents, created_at FROM photos");
  } catch (e) {
    // Handle old schema or missing table robustly
    if (e?.code === 'ER_NO_SUCH_TABLE') {
      try { await conn.execute(`CREATE TABLE IF NOT EXISTS photos (id INT AUTO_INCREMENT PRIMARY KEY, url VARCHAR(1024) NOT NULL, price_cents INT NOT NULL DEFAULT 0, created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4`) } catch {}
      rows = []
    } else {
      try { await conn.execute('ALTER TABLE photos ADD COLUMN price_cents INT NOT NULL DEFAULT 0') } catch {}
      const r = await conn.execute("SELECT id, url, price_cents, created_at FROM photos");
      rows = r[0]
    }
  }
